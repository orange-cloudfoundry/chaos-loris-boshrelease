...
export pipeline=${pipeline:-chaos-loris-boshrelease}
...
pushd $DIR
 ...
 fly -t ${fly_target} set-pipeline -c pipeline.yml -p ${pipeline} --load-vars-from $BOSH_CREDENTIALS_DIR/ci/deploy/credentials-chaos-loris-boshrelease.yml --non-interactive
popd
echo "Done"
